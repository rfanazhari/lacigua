...
class SprintPermataVA extends \App\Library\GlobalFunctionsCall
{
	var $ChannelID = 'LACIGPVA01';
	...
	public function _response($request, $SprintPermatava) {
		$data_sprint = array(
		...
		$authCode = hash("sha256", $SprintPermatava->TransactionCode.$SprintPermatava->GrandTotal.$this->ChannelID.$request['transactionStatus'].$SprintPermatava->InsertID.$this->SecretKey);
		...
		} elseif( $data_sprint['transactionNo'] != $SprintPermatava->TransactionCode ) {
		    $response['paymentStatus']  = "01";
		    $response['paymentMessage'] = "Invalid Currency";
		} elseif( $data_sprint['transactionAmount'] != $SprintPermatava->GrandTotal ) {
		    $response['paymentStatus']  = "01";
