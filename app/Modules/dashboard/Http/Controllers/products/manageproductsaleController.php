...
class manageproductsaleController extends Controller
{
    // Set header active
    public $header = [
        ...
        'addnew'    => false,
        'refresh'   => true,
        'downloadbulkproductsalebeauty' => true,
        'downloadbulkproductsalefashion' => true,
        'updatebulkproductsale' => true,
    ];
    ...
    public $aliasform = [
        ...
        'permalink' => ['permalink'],
        'StyleList' => ['Style'],
        'updatebulkproductsale' => ['Update Bulk Product Sale'],
    ];
    ...
    private function views($views = ["defaultview"], $arraywhere = []) {
        ...
        $result = $result->where(array_merge([['product.Status','=',0],['product.StatusSale','=',1]], $arraywhere))->orderBy($this->inv['flip'][$this->inv['getorder']], $this->inv['getsort']);
    ...
    public function downloadbulkproductsalebeauty()
    {
        $url = $this->_accessdata($this, __FUNCTION__);
        if($url) return $this->_redirect($url);
        ...
        $Product = $Product->where([['product.TypeProduct','=',1],['product.StatusSale','=',1],['product.Status','=',0]]);
        ...
        if($tmpProduct->get()) {
            ...
            $Title = "Bulk Product Sale Beauty";
    ...
    public function downloadbulkproductsalefashion()
    {
        $url = $this->_accessdata($this, __FUNCTION__);
        if($url) return $this->_redirect($url);
        ...
        $Product = $Product->where([['product.TypeProduct','=',0],['product.StatusSale','=',1],['product.Status','=',0]]);
        ...
        if($tmpProduct->get()) {
            ...
            $sheet = $objPHPExcel->setActiveSheetIndex(0);
            $Title = "Bulk Product Sale Fashion";
            $sheet->setTitle($Title);
    ...
    public function updatebulkproductsale()
    {
        $url = $this->_accessdata($this, __FUNCTION__);
        if($url) return $this->_redirect($url);
        $updatebulkproductsale = $errorupdatebulkproductsale = '';

        $requestfile = \Request::file();

        if(isset($requestfile['updatebulkproductsale'])) {
            $updatebulkproductsale = $requestfile['updatebulkproductsale'];

            $updatebulkproductsalename = $updatebulkproductsale->getPathName();
            $objPHPExcel = new \PHPExcel();

            try {
                $inputFileType = \PHPExcel_IOFactory::identify($updatebulkproductsalename);
                $objReader = \PHPExcel_IOFactory::createReader($inputFileType);
                $objPHPExcel = $objReader->load($updatebulkproductsalename);
            } catch(Exception $e) {
                $erroruploadbulkproduct = 'Error loading file "'.pathinfo($updatebulkproductsalename,PATHINFO_BASENAME).'": '.$e->getMessage();
            }
        ...
        $this->inv['updatebulkproductsale'] = $updatebulkproductsale; $this->inv['errorupdatebulkproductsale'] = $errorupdatebulkproductsale;
        return $this->_showview(["dashboard.products.updatebulkproductsale"]);
    }
}