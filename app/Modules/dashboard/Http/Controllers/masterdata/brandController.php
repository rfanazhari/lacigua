...
class brandController extends Controller
{
    ...
    public function delete()
    {
        ...
        if(isset($this->inv['delete']) && count($this->inv['delete'])) {
            ...
            foreach($this->inv['delete'] as $val) {
                $Brand = $this->Brand->where([[$this->objectkey,'=',$val]])->first();
                if($Brand) {
                    $this->Name = $Brand[$this->inv['flip']['Name']];
    ...
    private function views($views = ["defaultview"]) {
        ...
        $result = $this->Brand->leftJoin([
            ...
            'brand.*'
        ])->where([['brand.Status','=',0],['seller.IsActive','=',1],['seller.Status','=',0]])->orderBy($this->inv['flip'][$this->inv['getorder']], $this->inv['getsort']);
