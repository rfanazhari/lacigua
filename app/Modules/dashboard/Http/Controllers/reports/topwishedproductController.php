...
class topwishedproductController extends Controller
{
    // Set header active
    public $header = [
    ...
    public function index()
    {
        ...
        if(isset($request['download'])) {
            ...
            if(!$this->ReportDateFrom || !$this->ReportDateTo) {
                ...
                $ProductWishlist = $this->CustomerProductWishlist->leftjoin([
                    ['product as p', 'p.ID', '=', 'customer_product_wishlist.ProductID'],
                    ['customer as c', 'c.ID', '=', 'customer_product_wishlist.CustomerID'],
                    ['group_size as s', 's.ID', '=', 'customer_product_wishlist.GroupSizeID'],
                    ['size_variant as v', 'v.ID', '=', 'customer_product_wishlist.SizeVariantID'],
                    ['seller as m', 'm.ID', '=', 'p.SellerID'],
                    ['brand as b', 'b.ID', '=', 'p.BrandID'],
                    ['category as cat', 'cat.ID', '=', 'p.CategoryID'],
                    ['sub_category as subcat', 'subcat.IDCategory', '=', 'cat.ID'],
                ])->select([
                    ...
                    'p.ModelType as Model',
                    'm.FullName as SellerName',
                    'b.Name as BrandName',
                    'cat.Name as Category',
                    'subcat.Name as SubCategory',
                    's.Name as GroupSize',
                    'v.Name as SizeVariant',
                ])->groupBy('p.ID')->where('StatusWishlist', '=', '1');
                    
                if(is_numeric($this->SellerID)) {
                    $ProductWishlist = $ProductWishlist->where(function ($query) {
                        $query->where('m.ID', '=', $this->SellerID);
                    });
                }
                    
                if(is_numeric($this->BrandID)) {
                    $ProductWishlist = $ProductWishlist->where(function ($query) {
                        $query->where('b.ID', '=', $this->BrandID);
                    });
                }
                    
                if(is_numeric($this->ModelID)) {
                    $ProductWishlist = $ProductWishlist->where(function ($query) {
                        $query->where('p.ModelType', '=', $this->ModelID);
                    });
                }

                if(is_numeric($this->CategoryID)) {
                    $ProductWishlist = $ProductWishlist->where(function ($query) {
                        $query->where('cat.ID', '=', $this->CategoryID);
                    });
                }

                if(is_numeric($this->SubCategoryID)) {
                    $ProductWishlist = $ProductWishlist->where(function ($query) {
                        $query->where('subcat.ID', '=', $this->SubCategoryID);
                    });
                }

                $ProductWishlist = $ProductWishlist->where(function ($query) {
                    $query->whereBetween('customer_product_wishlist.CreatedDate', array($this->_dateformysql($this->ReportDateFrom), $this->_dateformysql($this->ReportDateTo)));
                });
                
                $ProductWishlist = $ProductWishlist;
                $tmpBookingOrder = $ProductWishlist;
                ...
                if($tmpBookingOrder->get()) {
                    ...
                    $loop = 2;
                    foreach ($ProductWishlist->get() as $obj) {
                        $loop2 = 0;
                        foreach ($arraytitle as $key => $value) {
                            $column = $obj->$key;
                            if(in_array($key, ['StatusWishlist'])) {
                                if(is_numeric($column)) $column = $arrwish[$column];
                            }
