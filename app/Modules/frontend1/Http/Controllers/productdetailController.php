        ...
        if(isset($request['ajaxpost'])) {
            switch($request['ajaxpost']) {
                case 'sendorder' :
                    ...
                    $GroupSizeID = $request['GroupSizeID'];
                ...
                case 'infochart' :
            		...
            		$this->_loaddbclass(['SizeChart']);

                    $SizeChart = $this->SizeChart->where([['GroupSizeID', '=', $GroupSizeID],['CategoryID', '=', $CategoryID],['SubCategoryID', '=', $SubCategoryID],['Status', '=', 0]])->first();
                    
            		if($SizeChart) {
            			die(json_encode(['response' => 'OK', 'data' => $this->inv['basesite'].'assets/frontend/images/content/sizechart/'.$SizeChart->Image], JSON_FORCE_OBJECT));
            		} else {
                ...
                case 'getsize' :
                    ...
                    if($Product->GroupSizeID == $GroupSizeID) {
                        ...
                        $ProductDetailSizeVariant = $this->ProductDetailSizeVariant->leftjoin([
                            ...
                            ['sv.CategoryID', '=', $CategoryID],
                            ['sv.SubCategoryID', '=', $SubCategoryID],
                    ...
                    if($ProductDetailSizeVariant) {
                        ...
                        $this->_loaddbclass(['SizeChart']);
                        $ListSizeChart = $this->SizeChart->where([['GroupSizeID', '=', $GroupSizeID],['CategoryID', '=', $CategoryID],['SubCategoryID', '=', $SubCategoryID],['Status', '=', 0]])->first();

                        if($ListSizeChart) $link_size = 'aaaaaa';
                    } else {
                        $response = 'Not OK';
                        $data = '';
                    }
...
<table>
	...
	public function index()
	{
		...
		if(isset($this->inv['getid'])) {
			$this->_loaddbclass(['Product', 'ProductLink', 'ProductDetailSizeVariant', 'GroupSize', 'SellerShipping']);
            ...
            $GroupSize = $this->GroupSize->leftjoin([
                ['size_variant as sv', 'sv.GroupSizeID', '=', 'group_size.ID']
            ])->where([
                ['sv.ModelType', '=', $Product->ModelType],
                ['sv.CategoryID', '=', $Product->CategoryID],
                ['sv.SubCategoryID', '=', $Product->SubCategoryID],
                ['group_size.IsActive','=',1],
                ['group_size.Status','=',0]
            ])->select(['group_size.*'])->groupBy('group_size.ID')->orderBy('group_size.ID', 'ASC')->get();
            $this->inv['GroupSize'] = $GroupSize;

            $this->_loaddbclass(['SizeChart']);

            $ListSizeChart = $this->SizeChart->where([['GroupSizeID', '=', $Product->GroupSizeID],['CategoryID', '=', $Product->CategoryID],['SubCategoryID', '=', $Product->SubCategoryID],['Status', '=', 0]])->first();
            $this->inv['ListSizeChart'] = $ListSizeChart;
			...
			return $this->_showviewfront(['productdetail']);
