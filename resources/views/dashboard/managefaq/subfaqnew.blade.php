			...
			<fieldset class='fieldset'><legend class='legend' rel="stylesheet">{{explode(' / ', $pagename)[count(explode(' / ', $pagename)) - 1]}} Information</legend>
				<div class="col-md-6">
					<div class="form-group">
						<div class="@if ($errorFaqID != '') has-error @endif">
							<label class="control-label input-sm marlm10">{{$alias['FaqID'][0]}} <span class="red">*</span></label>
							<select class="form-control input-sm select2me" name="FaqID">
								<option value=''>Select {{$alias['FaqID'][0]}}</option>
								@foreach ($arrfaq as $obj)
									<option value="{{$obj['ID']}}" @if ($obj['ID'] == $FaqID) selected @endif>{{$obj['Name']}}</option>
								@endforeach
							</select>
							<span class="help-block">{{$errorFaqID}}</span>
						</div>
					</div>
				</div>
				<div class="col-md-6">
