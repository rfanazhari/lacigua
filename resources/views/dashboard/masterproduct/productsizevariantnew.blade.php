	...
	<div class="form-body">
		<div class="row">
			<fieldset class='fieldset'><legend class='legend' rel="stylesheet">{{explode(' / ', $pagename)[count(explode(' / ', $pagename)) - 1]}} Information</legend>
				<div class="col-md-4">
					<div class="form-group">
						<div class="@if ($errorProductID != '') has-error @endif">
							...
							<select class="form-control input-sm select2me" name="ProductID" onchange="_getsize(this.value)">
				...
				<div class="col-md-4">
					<div class="form-group">
						<div class="@if ($errorSizeVariantID != '') has-error @endif">
							<label class="control-label input-sm marlm10">{{$alias['SizeVariantName'][0]}} <span class="red">*</span></label>
							<select class="form-control input-sm select2me" name="SizeVariantID">
								<option value=''>Select {{$alias['SizeVariantName'][0]}}</option>
								@foreach ($arrSizeVariant as $obj)
								<option value="{{$obj['ID']}}" @if ($obj['ID'] == $SizeVariantID) selected @endif> {{ $obj['Name'] }}
								</option>
								@endforeach
							</select>
							<span class="help-block">{{$errorSizeVariantID}}</span>
						</div>
					</div>
				</div>
				<div class="col-md-4">
...
<script type="text/javascript">
	function _getsize(obj) {
		$.ajax({
			...
			type		: 'POST',
			data		: {'ajaxpost':"_getsize",'value': obj},
			success		: function(data) {
				var data = JSON.parse(data);

				var listsize = "<option value=''>Select {{$alias['SizeVariantName'][0]}}</option>";
				if(data['response'] == 'OK') {
				...
				$('[name="SizeVariantID"]').html(listsize);
			}
		});
	}
</script>	