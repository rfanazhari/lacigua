...
<div id='table' class='table-scrollable'>
	<form id="defaultview" action="{{$basesite}}{{$config['backend']['aliaspage']}}{{$extlink}}" method='post'>
		<table class="bordered">
			<thead>
				<th class='pad10' style="width: 50px;">
					Edit
				</th>
				<th class='pad10' style="width: 80px;">
					{{ $alias['ModelType'][0] }}
				</th>
				<th class='pad10' style="width: 100px;">
					{{ $alias['CategoryImage'][0] }}
				</th>
				<th class='pad10'>
					{{ $alias['Name'][0] }}
				</th>
				<th class='pad10' style='width:130px;text-align: center;'>
					{{ $alias['ShowOnHeader'][0] }}
				</th>
				<th class='pad10' style='width:70px;'>
					{{ $alias['ColumnMode'][0] }}
				</th>
				<th class='pad10' style='width:130px;text-align: center;'>
					{{ $alias['ShowOnSubHeader'][0] }}
				</th>
			...
			<tfoot>
				...
				<td style="text-align: center;">
					<a href="{{$basesite}}{{$config['backend']['aliaspage']}}dashboard/masterproduct/productcategory/edit/id_{{$val['permalink']}}" class="buttonlink tooltips" data-original-title="Edit">
						<i class="fa fa-edit text-center"></i>
					</a>
				</td>
				<td>
					<i class="fa @if ($val['ModelType']) {{$val['ModelType']}} @endif"></i> {{$val['ModelType']}}
				</td>
				<td>
					<img src="{{ $basesite.'assets/frontend/images/content/category/small_'.$val['CategoryImage'] }}" alt="">
				</td>
				<td>
					<i class="fa @if ($val['Name']) {{$val['Name']}} @endif"></i> {{$val['Name']}}
				</td>
				<td>
				...
				<td>
					<p class="text-center">@if ($val['ColumnMode']) {{$val['ColumnMode']}} @endif</p>
				</td>
				<td>
					<input type="checkbox" class="make-switch SetOnSubHeader {{ $val['ID'] }}" data-on-text="Yes" data-off-text="No" data-on-color="success" data-off-color="danger" @if ($val['ShowOnSubHeader']) checked @endif rel="{{ $inv->_trans('dashboard.defaultview.buttonchangeonoff', ['value' => $alias['ShowOnSubHeader'][0]]) }}">
				</td>
				<td>
					<center>
						@if ($arrayfirst == $key)
