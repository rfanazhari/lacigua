	...
	<div class="form-body">
		<div class="row">
			<fieldset class='fieldset'><legend class='legend' rel="stylesheet">{{explode(' / ', $pagename)[count(explode(' / ', $pagename)) - 1]}} Information</legend>
				...
				<div class="col-md-4">
					<div class="form-group">
						<div class="@if ($errorSizeID != '') has-error @endif">
							<label class="control-label input-sm marlm10">{{$alias['SizeName'][0]}} <span class="red">*</span></label>
							<select class="form-control input-sm select2me" name="SizeVariantID">
								<option value=''>Select {{$alias['SizeName'][0]}}</option>
								@foreach ($arrSize as $obj)
								<option value="{{$obj['ID']}}" @if ($obj['ID'] == $SizeVariantID) selected @endif> {{ $obj['Name'] }}
								</option>
								@endforeach
							</select>
							<span class="help-block">{{$errorSizeID}}</span>
						</div>
					</div>
				</div>
				<div class="col-md-4">
