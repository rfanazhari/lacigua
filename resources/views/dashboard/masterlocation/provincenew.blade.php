				...
				<div class="col-md-6">
					<div class="form-group">
						<div class="@if ($errorCountryID != '') has-error @endif">
							<label class="control-label input-sm marlm10">{{$alias['CountryID'][0]}} <span class="red">*</span></label>
							<select class="form-control input-sm select2me" name="CountryID">
								<option value=''>Select {{$alias['CountryID'][0]}}</option>
								@foreach ($arrcountry as $obj)
									<option value="{{$obj['ID']}}" @if ($obj['ID'] == $CountryID) selected @endif>{{$obj['Name']}}</option>
								@endforeach
							</select>
