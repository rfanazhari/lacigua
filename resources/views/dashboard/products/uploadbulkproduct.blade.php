			...
			<fieldset class='fieldset'><legend class='legend' rel="stylesheet">{{explode(' / ', $pagename)[count(explode(' / ', $pagename)) - 1]}} Information</legend>
				<div class="col-md-12">
					<div class="form-group">
						<div id="error" class="fileinput fileinput-new @if ($erroruploadbulkproduct != '') has-error @endif" data-provides="fileinput">
							<label class="control-label input-sm marlm10">{{$alias['uploadbulkproduct'][0]}} <span class="red">*</span></label>
							<div id="uploadbulkproduct">
								@if (($action == 'edit' || $action == 'detail') && $uploadbulkproduct != '')
								<a href="{{$uploadbulkproduct}}" class="btn btn-sm blue">
									{{$alias['uploadbulkproduct'][0]}}
								</a>
								<a class="btn btn-sm red" value="Are you sure to remove {{$alias['uploadbulkproduct'][0]}} ?">
									Delete {{$alias['uploadbulkproduct'][0]}} <i class="fa fa-trash-o"></i>
								</a>
								@endif
							</div>
							<div class="input-group">
								...
								<span class="input-group-addon btn btn-sm default btn-file">
									...
									Select {{$alias['uploadbulkproduct'][0]}} </span>
									<span class="fileinput-exists">
									Change {{$alias['uploadbulkproduct'][0]}} </span>
									<input type="file" name="uploadbulkproduct">
								</span>
								<a href="#" class="input-group-addon btn btn-sm red fileinput-exists" data-dismiss="fileinput">
								Remove </a>
							</div>
							<span id="help" class="help-block">{{$erroruploadbulkproduct}}</span>
						</div>
					</div>
				</div>
			</fieldset>
