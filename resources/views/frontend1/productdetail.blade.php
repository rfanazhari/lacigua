	...
	<div class="wrapper" style="">
		...
		<div class="boxProductDetail after_clear">
			...
			<div class="boxImgDesc">
				...
				<div class="label_color">Color : <span id="data-color-name">{{$Product->ColorName}}</span></div>

				<ul class="change_color after_clear">
					@foreach($ArrProductLink as $obj)
					<li @if($obj['hexa'] == $Product->ColorHexa) class="active" @endif style="border:1px solid #dedede">
						@if($obj['hexa'] == $Product->ColorHexa)
						<a href="javascript:;" style="border:1px solid @if($obj['hexa'] == '#FFFFFF') #dedede @else #000 @endif;background-color: {{$obj['hexa']}}" ></a>
						@else
						<a href="{{$basesite}}product-detail/id_{{$obj['permalink']}}" style="border:1px solid @if($obj['hexa'] == '#FFFFFF') #dedede @else #000 @endif;background-color: {{$obj['hexa']}}" ></a>
						@endif
					</li>
					@endforeach
					<input type="hidden" name="color_text" id="color_text" value="{{$Product->ColorID}}">
				</ul>

				<div class="boxSize after_clear">
					<div class="boxSelect after_clear">
						<div class="label">Size : </div>
						<div class="inputsel">
							<select name="GroupSizeID" onchange="getsize(this.value, '{{$Product->ModelType}}', {{$Product->CategoryID}}, {{$Product->SubCategoryID}}, {{$Product->ID}})">
								@foreach($GroupSize as $obj)
								<option value="{{$obj->ID}}" @if($Product->GroupSizeID == $obj->ID) selected @endif>{{$obj->Name}}</option>
								@endforeach
							</select>
						</div>
					</div>
					<div id="link_size" class="link_size">
						@if($ListSizeChart)
						<a href="javascript:;" onclick="infochart()">Size Chart</a>
						@endif
					</div>
				</div>

				<ul class="circle_size" id="circle_size">
...
<script type="text/javascript">
	...
	function infochart() {
		var GroupSizeID = $('[name="GroupSizeID"]').val();
		$.ajax({
            url         : '{{$basesite}}product-detail/ajaxpost',
            type        : 'POST',
            data        : {'ajaxpost':"infochart",'_token':'{{csrf_token()}}','GroupSizeID':GroupSizeID,'CategoryID':{{$Product->CategoryID}},'SubCategoryID':{{$Product->SubCategoryID}}},
            success     : function(data) {
                var data = JSON.parse(data);
                if(data['response'] == 'OK') {
                	$('#infochart').attr('src', data['data']);
					openPopup('popup-infochart');
                }
            }
        });
	}
	...
	function ordershow() {
		...
		$('#popsize').html($('#circle_size').find("a.active").text());
	...
	function sendorder() {
		...
		if(!IDDistrict) _alertvalidation('<div class="control-label"><span class="colorred">*</span> Please Input Address</div>');
			...
			$.ajax({
	            url         : '{{$basesite}}product-detail/ajaxpost',
	            type        : 'POST',
	            data        : {'ajaxpost':"sendorder",'_token':'{{csrf_token()}}','ProductID':'{{$Product->ID}}','ColorID':$("#color_text").val(),'GroupSizeID':$('[name="GroupSizeID"]').val(),'SizeVariantID':$("#size_text").val(),'Qty':$('[name="Qty"]').val(),'ShippingID':ShippingID,'ShippingPackageID':ShippingPackageID,'IDCustomerAddress':$('#IDCustomerAddress').html(),'TextAddressInfo':$('#TextAddressInfo').html(),'TextRecepientName':$('#TextRecepientName').html(),'TextRecepientPhone':$('#TextRecepientPhone').html(),'TextAddress':$('#TextAddress').html(),'IDProvince':$('#IDProvince').html(),'TextDistrictName':$('#TextDistrictName').html(),'IDDistrict':IDDistrict,'TextCityName':$('#TextCityName').html(),'IDCity':$('#IDCity').html(),'TextPostalCode':$('#TextPostalCode').html(),'Notes':$('[name="Notes"]').val()},
	            success     : function(data) {
	                var data = JSON.parse(data);
	...
	function setwishlist() {
		$.ajax({
            url         : '{{$basesite}}product-detail/ajaxpost',
            type        : 'POST',
            data        : {'ajaxpost':"setwishlist",'_token':'{{csrf_token()}}','ID':'{{$Product->ID}}','GroupSizeID':$('[name="GroupSizeID"]').val(),'SizeVariantID':$("#size_text").val()},
            success     : function(data) {
                var data = JSON.parse(data);
	...
	function getsize(GroupSizeID, ModelType, CategoryID, SubCategoryID, ProductID) {
		$.ajax({
            url         : '{{$basesite}}product-detail/ajaxpost',
            type        : 'POST',
            data        : {'ajaxpost':"getsize",'_token':'{{csrf_token()}}','GroupSizeID':GroupSizeID,'ModelType':ModelType,'CategoryID':CategoryID,'SubCategoryID':SubCategoryID,'ProductID':ProductID},
            success     : function(data) {
                var data = JSON.parse(data);
                ...
                if(data['response'] == 'OK') {
					...
					$.each(data['data'], function(key, val) {
                		...
                		circle_size = circle_size + '<li><a href="javascript:;" data-size="'+val['SizeVariantID']+'" '+active+' >'+val['SizeName']+'</a></li>';
                		arrsize[val['SizeVariantID']] = val['SizeQty'];
                		i = i + 1;
                	});
                	
                	if(circle_size) {
	            		circle_size = circle_size + '<input type="hidden" name="size_text" id="size_text" value="'+data['data'][0]['SizeVariantID']+'">';

	            		$('#circle_size').html(circle_size);
	...
	$("#circle_size").on("click","li a", function() {
		...
		$("#size_text").val(size);
		$("#circle_size li a").removeClass("active");
